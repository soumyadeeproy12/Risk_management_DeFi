...
import { PollingWatchKind } from "typescript";
    ...
    const pool_info =  async () => {
        ...
        console.log(underlying_balance)
            if (underlying_balance[0] < 0.70*underlying_balance_init[0]) {
                console.log("Alert: Liquidity has gone down by 30% of "+underlying[0]);
            }
            if(underlying_balance[1] < 0.70*underlying_balance_init[1]) {
                console.log("Alert: Liquidity has gone down by 30% of "+underlying[1]);
            }
            if(underlying_balance[2] < 0.70*underlying_balance_init[2]) {
                console.log("Alert: Liquidity has gone down by 30% of "+underlying[2]);
            }
            if(underlying_balance[3] < 0.70*underlying_balance_init[3]) {
                console.log("Alert: Liquidity has gone down by 30% of "+underlying[3]);
            };
            if((underlying_balance[0] >0.70*underlying_balance_init[0]) || (underlying_balance[1] >0.70*underlying_balance_init[1]) || (underlying_balance[2] >0.70*underlying_balance_init[2]) || (underlying_balance[3] >0.70*underlying_balance_init[3])) 
            {
               console.log("Everything is alright with the underlying coins (not less than 30%): "+underlying[0]+","+ underlying[1]+","+underlying[2]+","+underlying[3])
            };
